...
impl<T: Config> Pallet<T> {
	...
	/// Fetch cross chain events
	pub(super) fn get_locked_events(
		relay_contract: Vec<u8>,
		appchain_id: Vec<u8>,
		start: u32,
		limit: u32,
	) -> Result<Vec<LockEvent<<T as frame_system::Config>::AccountId>>, http::Error> {
		// We want to keep the offchain worker execution time reasonable, so we set a hard-coded
		// deadline to 2s to complete the external call.
		// You can also wait idefinitely for the response, however you may still get a timeout
		// coming from the host machine.
		...
		// since we are running in a custom WASM execution environment we can't simply
		// import the library here.
		let args = Self::encode_events_args(appchain_id, start, limit).ok_or_else(|| {
			log::info!("🐙 Encode args error");
			http::Error::Unknown
		})?;
		...
		log::info!("🐙 Got lock events: {:?}", events);

		Ok(events)
	}

	fn encode_events_args(appchain_id: Vec<u8>, start: u32, limit: u32) -> Option<Vec<u8>> {
		let a = String::from("{\"appchain_id\":\"");
		let appchain_id = sp_std::str::from_utf8(&appchain_id).expect("octopus team will ensure that the appchain_id of a live appchain is a valid UTF8 string; qed");
